...
|------|------|
| 요구사항이 모호함 | ❌ 추측 → ✅ 질문 |
| 여러 해석 가능 | ❌ 하나 선택 → ✅ 옵션 제시 후 질문 |
| 기존 설계 의도 불명확 | ❌ 추론 → ✅ "~로 보이는데 맞나요?" |
| 영향 범위 불확실 | ❌ 진행 → ✅ 확인된 부분만 보고 후 질문 |

```
❓ 확인 필요
   상황: 설정 저장 방식이 2가지로 보입니다
   옵션 1: 전체 저장 (SettingsModal)
   옵션 2: 개별 저장 (각 컴포넌트)
   → 어떤 방식을 원하시나요?
```
