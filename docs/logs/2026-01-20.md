...
### Tracking_Session 테이블 빈 컬럼 수정
- **유형**: Bugfix
- **영향**: Backend (EphemerisDataRepository, PassScheduleDataRepository)
- **요약**: 키 이름 불일치로 인한 null 저장 문제 해결

### Why
- MST 데이터 키(SatelliteID, MaxAzRate 등)와 Repository 매핑 키 불일치
- satellite_id, duration, max_azimuth_rate, max_elevation_rate, total_points 컬럼이 null로 저장됨

### 구현 내용

| 파일 | 변경 |
|------|------|
| EphemerisDataRepository.kt | mapMstToSession() 키 매핑 수정 |
| PassScheduleDataRepository.kt | mapMstToSession() 키 매핑 수정 |

**수정된 매핑:**
- `SatelliteID` || `SatelliteId` → satellite_id
- `MaxAzRate` || `MaxAzimuthRate` → max_azimuth_rate
- `MaxElRate` || `MaxElevationRate` → max_elevation_rate
- `IsKeyhole` || `KeyholeDetected` → keyhole_detected
- Duration ISO 문자열 파싱 추가 (parseDurationToSeconds)

### 산출물
- [Tracking_Session_Data_Enrichment](../work/active/Tracking_Session_Data_Enrichment/)

### 상태
- ✅ 구현 완료 (BE 빌드 성공)
- ⏳ DB 저장 테스트 대기 (BE 재시작 필요)

---

### HW_Error 로깅 컨텍스트 추가
- **유형**: Feature
- **영향**: Backend (HardwareErrorLogService, HardwareErrorLogRepository)
- **요약**: HW 에러 로그에 추적 모드, 상관 ID, 원시 데이터 추가

### Why
- 에러 발생 시 어떤 모드(Standby, PassSchedule 등)에서 발생했는지 추적 불가
- 동시에 발생한 에러들을 그룹화하여 분석하기 어려움

### 구현 내용

| 컬럼 | 설명 |
|------|------|
| tracking_mode | 에러 발생 시 현재 추적 모드 |
| correlation_id | 동시 에러 그룹화 UUID |
| raw_data | 원시 비트 데이터 JSON |

### 산출물
- [HW_Error_System_Integration](../work/active/HW_Error_System_Integration/)
...
- **유형**: Database
- **영향**: Flyway Migration
- **요약**: 테이블 및 컬럼에 설명 코멘트 추가

### Why
- DBeaver 등 DB 도구에서 테이블/컬럼 설명 확인 가능
- 팀원 온보딩 시 스키마 이해도 향상

### 산출물
- `V003__Add_table_column_comments.sql`
- `FlywayConfig.kt` (컴포넌트 분리)
