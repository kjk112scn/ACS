...
| Phase | 상태 | 진행률 | 완료/전체 | 남은 항목 |
|-------|:----:|:------:|:---------:|-----------|
| **Sprint 0** | ⬜ 대기 | 0% | 0/3 | S0-1,2,3 |
| **Phase 1** | ⬜ 대기 | 0% | 0/6 | P1-1~6 |
| **Phase 2** | ⬜ 대기 | 0% | 0/3 | P2-1,2,3 |
| **Phase 3** | 🔄 진행 | 25% | 1/4 | P3-1,2,4 |
| **Phase 4** | ⬜ 대기 | 0% | 0/2 | P4-1,2 |
| **Phase 5** | ⬜ 대기 | 0% | 0/2 | P5-1,2 |

### 완료된 작업

| 날짜 | 항목 | 설명 |
|------|------|------|
| 2026-01-16 | P3-3 | icdStore.ts Composable 패턴 분리 |

### 남은 작업 요약 (19개)

| 우선순위 | 항목 | 난이도 | 예상 시간 |
|:--------:|------|:------:|:---------:|
| 🔥 Critical | S0-1 Path Traversal | 🟢 | 15분 |
| 🔥 Critical | S0-2 CORS Wildcard | 🟢 | 10분 |
| 🔥 Critical | S0-3 innerHTML XSS | 🟡 | 1시간 |
| ⬜ Phase 1 | P1-1~P1-6 | 🟢🟡 | 8시간 |
| ⬜ Phase 2 | P2-1 deep watch | 🔴 | 6시간 |
| ⬜ Phase 2 | P2-2 console.log | 🟡 | 4시간 |
| ⬜ Phase 2 | P2-3 icdStore 최적화 | 🔴 | 6시간 |
| ⬜ Phase 3 | P3-1 PassSchedulePage | 🔴🔴 | 8시간 |
| ⬜ Phase 3 | P3-2 EphemerisPage | 🔴 | 6시간 |
| ⬜ Phase 3 | P3-4 BE 서비스 분리 | 🔴🔴 | 10시간 |
| ⬜ Phase 4 | P4-1 @Valid 검증 | 🟢 | 2시간 |
| ⬜ Phase 4 | P4-2 catch(Exception) | 🟡 | 8시간 |
| ⬜ Phase 5 | P5-1,2 키보드 단축키 | 🟢 | 2시간 |
...
Today: 1/16 (금) ◀━━ 현재
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1/15 (목) │ Sprint 0 + Phase 1 │ ⬜ 미시작
          │                     │
1/16 (금) │ P2-1 deep watch    │ ⬜ 미시작  ◀━━ TODAY
          │ P2-2 console.log   │
          │                     │
1/17 (토) │ P2-3 icdStore      │ ⬜ 예정
          │ P4-1, P4-2 시작    │
          │                     │
1/18 (일) │ P4-2 완료          │ ⬜ 예정
          │ P3-1 PassSchedule  │
          │                     │
1/19 (월) │ P3-2 Ephemeris     │ ⬜ 예정
          │ P5 키보드 단축키   │
          │                     │
1/20 (화) │ P3-4 BE 서비스     │ ⬜ 예정
          │                     │
1/21 (수) │ 버퍼 / 마무리      │ ⬜ 예정
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
          │ Target: 1/21 완료  │
...
| 항목 | 원래 일정 | 현재 상태 | 지연 |
|------|----------|----------|:----:|
| Sprint 0 | 1/15 오전 | 미시작 | ⚠️ 1일 |
| Phase 1 | 1/15 저녁 | 미시작 | ⚠️ 1일 |
| Phase 2 | 1/16~17 | 미시작 | - |
| Phase 3 | 1/18~19 | P3-3만 완료 | - |
...
> **완료 (2026-01-16)**: Composable 패턴으로 분리 완료
> **변경 이유**: 스토어 분리 시 순환 의존성 위험, Pinia 설계 원칙 위반 가능성

```
결과: stores/icd/
├── index.ts (re-export)
├── icdStore.ts (~2,800줄)          ← 단일 스토어 유지
├── composables/
│   ├── index.ts                    ← re-export
│   ├── useBoardStatus.ts (~215줄)  ← 14개 비트 파싱 순수 함수
│   ├── useAntennaState.ts (~450줄) ← 안테나 데이터 타입 및 헬퍼
│   └── useTrackingState.ts (~160줄)← 추적 상태 업데이트 순수 함수
└── types/
    ├── index.ts                    ← re-export
    └── icdTypes.ts (~200줄)        ← 타입 정의 분리
```

**적용된 변경**:
- ✅ `types/icdTypes.ts` - 타입 정의 분리
- ✅ `composables/useBoardStatus.ts` - 14개 파싱 함수 순수 함수화
- ✅ `composables/useAntennaState.ts` - 안테나 데이터 타입/헬퍼 분리
- ✅ `composables/useTrackingState.ts` - 추적 상태 업데이트 순수 함수화
- ✅ `icdStore.ts` - 순수 함수 import 및 적용

**Composable 패턴 장점** (검증됨):
- 순환 의존성 없음 (순수 함수, 스토어가 ref 주입)
- Pinia 단일 스토어 원칙 유지
- 점진적 마이그레이션 완료
- 독립 테스트 용이
