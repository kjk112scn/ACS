...
├── README.md                          # 문서 시스템 안내
│
├── architecture/context/                          # 🆕 개념 문서 (영구 보존)
...
워크플로우:
  ...
  5. 완료 처리 (/done)
     ...
     - CHANGELOG.md 업데이트
     - architecture/context/ 문서 업데이트 (해당 시)
```
...
```yaml
/done 실행 시:
  - work/active/{기능}/ → work/archive/{기능}/
  - work/active/{버그}/ → work/archive/{버그}/
  - PROGRESS.md → IMPLEMENTATION.md로 변환
  - 관련 architecture/context/ 문서 업데이트

/sync 실행 시:
  - 코드 변경 → api/ 문서 자동 업데이트
  - 새 Controller → api/controllers/ 문서 생성
  - 새 Algorithm → architecture/context/algorithms/ 문서 업데이트
```
...
작업:
  ...
  2. 스킬 간 연계 자동화
  3. ADR 자동 생성 로직
  4. architecture/context/ 문서 자동 업데이트
```
