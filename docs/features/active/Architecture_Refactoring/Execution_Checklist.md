...
- [ ] API Key 인증 구현
- [ ] JWT 토큰 발급
- [ ] Role 기반 인가 (Admin/Supervisor/Observer)
...
#### BE 코드 품질 (심층 분석 결과)
- [ ] !! 연산자 제거 (46건)
  - [ ] EphemerisService.kt (12건)
  - [ ] PassScheduleService.kt (8건)
  - [ ] ICDService.kt (7건)
  - [ ] 기타 파일들 (19건)
- [ ] 매직 넘버 상수화 (40+건)
  - [ ] 타임아웃 값 상수화
...
#### Phase 7: icdStore 심층 최적화 (P0) [신규 - Critical!]

> **배경**: ICD 부분 코드 복잡, 30ms마다 175개 ref 업데이트로 성능 저하

##### 개별 ref → shallowRef 그룹화 (175회 → 5-10회)
- [ ] AntennaState 인터페이스 정의
  - [ ] angles 그룹 (azimuth, elevation, train)
  - [ ] speeds 그룹
  - [ ] boardStatus 그룹
- [ ] antennaState = shallowRef<AntennaState>() 생성
- [ ] 기존 175개 개별 ref 마이그레이션
- [ ] updataAntennaData 함수 리팩토링 (338줄 → ~100줄)

##### 비트 파싱 함수 배치 처리 (13회 → 1회)
- [ ] BoardStatus 인터페이스 정의
- [ ] parseAllBoardStatus 함수 생성
- [ ] 기존 13개 파싱 함수 통합
- [ ] 72개 개별 ref → shallowRef 변환

##### Computed 최적화 (22개)
- [ ] 대형 computed 분리 (필요 시에만 계산)
- [ ] filter/배열 생성 최소화
- [ ] 불필요한 속성 제거
...
| 타입 단언(as) | 80+건 | **280건** | High | RFC-008 Phase 4 |
| 하드코딩 색상 | 300+건 | **520건** | High | RFC-008 Phase 4 |
...
- [ ] `stores/common/modeStore.ts` vs `stores/icd/modeStore.ts` - 통합 필요
