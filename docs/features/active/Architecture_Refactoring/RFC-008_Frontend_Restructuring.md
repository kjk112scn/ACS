...
> **버전**: 1.4.0 | **작성일**: 2026-01-13
> **상태**: Draft | **우선순위**: P2
> **역할**: FE 전체 구조화 (Composable 추출, 대형 파일 분리, console.log 정리, 코드 품질, 성능 최적화, icdStore 최적화, Dead Code 정리, 메모리 누수 수정, CSS 품질)
...
| 디버깅 코드 잔재 | console.log 988개, 번들 크기 증가 |
...
| 파일 | 개수 | 비율 |
|------|------|------|
| PassSchedulePage.vue | 128 | 13% |
| passScheduleStore.ts | 103 | 10% |
| SelectScheduleContent.vue | 80 | 8% |
| TLEUploadContent.vue | 64 | 6% |
| EphemerisDesignationPage.vue | 63 | 6% |
| DashboardPage.vue | 60 | 6% |
| 기타 | 490 | 51% |
| **합계** | **988** | 100% |
...
### 2.5 코드 품질 이슈 (심층 분석 결과)

#### A. 하드코딩 색상 - 300+건 (CLAUDE.md 위반!)

> **CLAUDE.md 규칙**: "색상은 테마 변수 사용: `var(--theme-*)`, 하드코딩 금지"

| 파일 | 건수 | 예시 |
|------|------|------|
| PassSchedulePage.vue | 45+ | `color: '#FF6B6B'` |
| EphemerisDesignationPage.vue | 38+ | `backgroundColor: 'rgba(66, 133, 244, 0.1)'` |
| DashboardPage.vue | 35+ | `borderColor: '#4CAF50'` |
| SelectScheduleContent.vue | 28+ | `fill: '#2196F3'` |
| 기타 | 150+ | - |
...
| 파일 | 건수 | 위험도 |
|------|------|--------|
| icdStore.ts | 22건 | High (런타임 오류 위험) |
| passScheduleStore.ts | 15건 | Medium |
| EphemerisDesignationPage.vue | 12건 | Medium |
| 기타 | 31건 | Low |
...
**즉시 (P0)**:
```typescript
// shallowRef로 그룹화 (175회 → 1회)
interface AntennaState {
  angles: { azimuth: string; elevation: string; train: string }
  speeds: { azimuth: string; elevation: string; train: string }
  boardStatus: { /* 비트 상태 그룹화 */ }
}
const antennaState = shallowRef<AntennaState>({ /* 초기값 */ })

// 업데이트: 단일 반응성 트리거
antennaState.value = { ...antennaState.value, ...newData }
```

**단기 (P1)**:
```typescript
// 비트 파싱 배치 처리 (13회 호출 → 1회)
interface BoardStatus {
  protocol: ProtocolStatus
  mainBoard: MainBoardStatus
  // ...
}
const parsedBoardStatus = shallowRef<BoardStatus>(defaultStatus)

function parseAllBoardStatus(data: Record<string, string>) {
  parsedBoardStatus.value = {
    protocol: parseProtocolBits(data.protocol),
    mainBoard: parseMainBoardBits(data.mainBoard),
    // ... 배치 처리
  }
}
```
...
- Worker Thread로 비트 파싱 분리

#### G. 예상 효과

| 항목 | 현재 | 최적화 후 |
|------|------|----------|
| ref 업데이트/30ms | 175회 | 5-10회 |
| 비트 파싱 함수 호출 | 13회 | 1회 |
| computed 재계산 | 전체 22개 | 필요한 것만 |
| 예상 CPU 감소 | - | **70-80%** |
...
| 파일 | 위치 | 문제 | 심각도 |
|------|------|------|--------|
| ExampleComponent.vue | components/ | 미사용 예제 컴포넌트 | Critical |
| example-store.ts | stores/ | 미사용 예제 스토어 | Critical |
| modeStore 중복 | stores/ | **2개 파일** 동일 역할 | High |
...
| 타입 단언(as) | 80+건 | **280건** | +200 |
| 하드코딩 색상 | 300+건 | **520건** | +220 |
...
| 항목 | 결정 | 비고 |
|------|------|------|
| Composable 추출 | 4개 신규 생성 | useTrackingState 우선 |
| 대형 파일 분리 | 기능별 분리 | P1 이후 진행 |
| console.log | devLog로 통합 | 1,513개 → 개발 모드만 |
| Dead Code | 즉시 삭제 | 3건 (Critical) |
